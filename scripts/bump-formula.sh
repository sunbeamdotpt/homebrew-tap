...
[ -f "$formula_file" ] || { echo "error: no such file: $formula_file" >&2; exit 66; }
...
first_url_line=$(grep -m1 -E '^[[:space:]]*url "' "$formula_file") \
  || { echo "error: no url line found in $formula_file" >&2; exit 65; }

current_version=$(echo "$first_url_line" | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1)
current_version="${current_version#v}"
[ -n "$current_version" ] || { echo "error: could not parse version from: $first_url_line" >&2; exit 65; }
...
[ -n "$repo" ] || { echo "error: could not parse github repo from homepage in $formula_file" >&2; exit 65; }
...
while read -r url_lineno; do
  old_url=$(sed -n "${url_lineno}s/.*url \"\(.*\)\".*/\1/p" "$formula_file")
  new_url=${old_url//${current_version}/${new_version}}
  [ "$new_url" != "$old_url" ] || { echo "error: version $current_version not found in $old_url" >&2; exit 65; }

  filename=${new_url##*/}
  sha=$(lookup_sha "$filename")
  if [ -z "$sha" ]; then
    echo "  $filename: not in checksums.txt, downloading to hash"
    curl -fsSL "$new_url" -o "$tmpdir/asset" || { echo "error: download failed: $new_url" >&2; exit 65; }
    sha=$(sha256_of "$tmpdir/asset")
    rm -f "$tmpdir/asset"
  else
    echo "  $filename: sha256 from checksums.txt"
  fi
  [ -n "$sha" ] || { echo "error: could not determine sha256 for $new_url" >&2; exit 65; }
  ...
  [ -n "$sha_lineno" ] || { echo "error: no sha256 line after url at line $url_lineno" >&2; exit 65; }
  next_url_lineno=$(awk -v start="$url_lineno" 'NR > start && /url "/ { print NR; exit }' "$formula_file")
  if [ -n "$next_url_lineno" ] && [ "$sha_lineno" -gt "$next_url_lineno" ]; then
